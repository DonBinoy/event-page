...
/* ─── REVEAL ─────────────────────────────────────── */
export function Rev({ children, delay = 0, style = {} }: { children: React.ReactNode; delay?: number; style?: React.CSSProperties }) {
  const { tokens: { A, AH, AL, BG, FG, M, S, B, W }, theme, toggleTheme } = useTheme();
...
/* ─── CHAR SPLIT ─────────────────────────────────── */
export function Chars({ text, cls = "", style = {}, delay = 0 }: { text: string; cls?: string; style?: React.CSSProperties; delay?: number }) {
  const { tokens: { A, AH, AL, BG, FG, M, S, B, W }, theme, toggleTheme } = useTheme();
  const r = useRef<HTMLDivElement>(null);
...
/* ─── MARQUEE ────────────────────────────────────── */
export function Mq({ items, dir = "l", size = "sm", bg, accent = false }: { items: string[]; dir?: "l" | "r"; size?: "sm" | "lg" | "xl"; bg?: string; accent?: boolean }) {
  const { tokens: { A, AH, AL, BG, FG, M, S, B, W }, theme, toggleTheme } = useTheme();
  const bgColor = bg ?? BG;
  const sep = "  ·  ";
...
/* ─── SECTION HEADER ─────────────────────────────── */
export function SHdr({ idx, label }: { idx: string; label: string }) {
  const { tokens: { A, AH, AL, BG, FG, M, S, B, W }, theme, toggleTheme } = useTheme();
  return (
